<template>
    <form @submit.prevent="submit">
        ...
        <div class="form-group">
            <label for="email">{{ translate('messages.E-mail' )}}</label>
            <input type="email" class="form-control" name="email" id="email" v-model="fields.email" />
            <div v-if="errors && errors.email" class="text-danger">{{ errors.email[0] }}</div>
        </div>
        <div class="form-group">
            <label for="message">{{ translate('messages.Message' )}}</label>
            <textarea class="form-control" id="message" name="message" rows="5" v-model="fields.message"></textarea>
            <div v-if="errors && errors.message" class="text-danger">{{ errors.message[0] }}</div>
        </div>

        <button type="submit" class="btn btn-primary">{{ translate('messages.Send message' )}}</button>

         <div v-if="success" class="alert alert-success mt-3">
            {{ translate('messages.Message sent!' )}}
        </div>
    </form>
</template>
